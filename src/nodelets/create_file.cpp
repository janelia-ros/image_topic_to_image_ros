...
class CreateFileNodelet : public nodelet::Nodelet
{
  // ROS communication
...
void CreateFileNodelet::onInit()
{
  ...
  image_transport::SubscriberStatusCallback connect_cb = boost::bind(&CreateFileNodelet::connectCb,this);
  ros::SubscriberStatusCallback connect_cb_info = boost::bind(&CreateFileNodelet::connectCb,this);
...
void CreateFileNodelet::connectCb()
{
  boost::lock_guard<boost::mutex> lock(connect_mutex_);
...
void CreateFileNodelet::publish()
{
  ros::NodeHandle & nh = getNodeHandle();
...
PLUGINLIB_EXPORT_CLASS( image_stream_to_heatmap::CreateFileNodelet,nodelet::Nodelet)
